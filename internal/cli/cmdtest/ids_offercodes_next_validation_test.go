...
func TestOfferCodesListRejectsInvalidNextURL(t *testing.T) {
	tests := []struct {
		name    string
		next    string
		wantErr string
		...
		{
			name:    "invalid scheme",
			next:    "http://api.appstoreconnect.apple.com/v1/subscriptionOfferCodes/offer-1/oneTimeUseCodes?cursor=AQ",
			wantErr: "offer-codes list: --next must be an App Store Connect URL",
		},
		{
			name:    "malformed URL",
			next:    "https://api.appstoreconnect.apple.com/%zz",
			wantErr: "offer-codes list: --next must be a valid URL:",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			var runErr error
			stdout, stderr := captureOutput(t, func() {
				if err := root.Parse([]string{"offer-codes", "list", "--next", test.next}); err != nil {
					t.Fatalf("parse error: %v", err)
				}
				runErr = root.Run(context.Background())
			})
...
func TestOfferCodesListPaginateFromNextWithoutOfferCode(t *testing.T) {
	setupAuth(t)
	t.Setenv("ASC_CONFIG_PATH", filepath.Join(t.TempDir(), "nonexistent.json"))
	...
	http.DefaultTransport = roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		case 1:
			...
			body := `{"data":[{"type":"subscriptionOfferCodeOneTimeUseCodes","id":"otc-next-1"}],"links":{"next":"` + secondURL + `"}}`
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(body)),
				Header:     http.Header{"Content-Type": []string{"application/json"}},
		...
		case 2:
			...
			body := `{"data":[{"type":"subscriptionOfferCodeOneTimeUseCodes","id":"otc-next-2"}],"links":{"next":""}}`
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(body)),
				Header:     http.Header{"Content-Type": []string{"application/json"}},
	...
	stdout, stderr := captureOutput(t, func() {
		if err := root.Parse([]string{"offer-codes", "list", "--paginate", "--next", firstURL}); err != nil {
			t.Fatalf("parse error: %v", err)
		}
		if err := root.Run(context.Background()); err != nil {
			t.Fatalf("run error: %v", err)
...
func TestOfferCodesCustomCodesListRejectsInvalidNextURL(t *testing.T) {
	tests := []struct {
		name    string
		next    string
		wantErr string
		...
		{
			name:    "invalid scheme",
			next:    "http://api.appstoreconnect.apple.com/v1/subscriptionOfferCodes/offer-1/customCodes?cursor=AQ",
			wantErr: "offer-codes custom-codes list: --next must be an App Store Connect URL",
		},
		{
			name:    "malformed URL",
			next:    "https://api.appstoreconnect.apple.com/%zz",
			wantErr: "offer-codes custom-codes list: --next must be a valid URL:",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			var runErr error
			stdout, stderr := captureOutput(t, func() {
				if err := root.Parse([]string{"offer-codes", "custom-codes", "list", "--next", test.next}); err != nil {
					t.Fatalf("parse error: %v", err)
				}
				runErr = root.Run(context.Background())
			})
...
func TestOfferCodesCustomCodesListPaginateFromNextWithoutOfferCodeID(t *testing.T) {
	setupAuth(t)
	t.Setenv("ASC_CONFIG_PATH", filepath.Join(t.TempDir(), "nonexistent.json"))
	...
	http.DefaultTransport = roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		case 1:
			...
			body := `{"data":[{"type":"subscriptionOfferCodeCustomCodes","id":"custom-next-1"}],"links":{"next":"` + secondURL + `"}}`
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(body)),
				Header:     http.Header{"Content-Type": []string{"application/json"}},
		...
		case 2:
			...
			body := `{"data":[{"type":"subscriptionOfferCodeCustomCodes","id":"custom-next-2"}],"links":{"next":""}}`
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(body)),
				Header:     http.Header{"Content-Type": []string{"application/json"}},
	...
	stdout, stderr := captureOutput(t, func() {
		if err := root.Parse([]string{"offer-codes", "custom-codes", "list", "--paginate", "--next", firstURL}); err != nil {
			t.Fatalf("parse error: %v", err)
		}
		if err := root.Run(context.Background()); err != nil {
			t.Fatalf("run error: %v", err)
...
func TestOfferCodesPricesListRejectsInvalidNextURL(t *testing.T) {
	tests := []struct {
		name    string
		next    string
		wantErr string
		...
		{
			name:    "invalid scheme",
			next:    "http://api.appstoreconnect.apple.com/v1/subscriptionOfferCodes/offer-1/prices?cursor=AQ",
			wantErr: "offer-codes prices list: --next must be an App Store Connect URL",
		},
		{
			name:    "malformed URL",
			next:    "https://api.appstoreconnect.apple.com/%zz",
			wantErr: "offer-codes prices list: --next must be a valid URL:",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			var runErr error
			stdout, stderr := captureOutput(t, func() {
				if err := root.Parse([]string{"offer-codes", "prices", "list", "--next", test.next}); err != nil {
					t.Fatalf("parse error: %v", err)
				}
				runErr = root.Run(context.Background())
			})
...
func TestOfferCodesPricesListPaginateFromNextWithoutOfferCodeID(t *testing.T) {
	setupAuth(t)
	t.Setenv("ASC_CONFIG_PATH", filepath.Join(t.TempDir(), "nonexistent.json"))
	...
	http.DefaultTransport = roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		case 1:
			...
			body := `{"data":[{"type":"subscriptionOfferCodePrices","id":"price-next-1"}],"links":{"next":"` + secondURL + `"}}`
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(body)),
				Header:     http.Header{"Content-Type": []string{"application/json"}},
		...
		case 2:
			...
			body := `{"data":[{"type":"subscriptionOfferCodePrices","id":"price-next-2"}],"links":{"next":""}}`
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(body)),
				Header:     http.Header{"Content-Type": []string{"application/json"}},
	...
	stdout, stderr := captureOutput(t, func() {
		if err := root.Parse([]string{"offer-codes", "prices", "list", "--paginate", "--next", firstURL}); err != nil {
			t.Fatalf("parse error: %v", err)
		}
		if err := root.Run(context.Background()); err != nil {
			t.Fatalf("run error: %v", err)
